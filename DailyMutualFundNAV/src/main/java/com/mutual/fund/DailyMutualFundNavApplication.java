...
public class DailyMutualFundNavApplication {
	...
	public Docket configureSwagger() {
		return new Docket(DocumentationType.SWAGGER_2).select().paths(PathSelectors.ant("/**"))
				.apis(RequestHandlerSelectors.basePackage("com.mutual.fund.controller")).build().apiInfo(apiInfo());
	}

	private ApiInfo apiInfo() {
		return new ApiInfo("Daily Mutual Fund NAV Api",
				"Daily Mutual Fund NAV with Authenticating User and check for validity of token", "1.0", "path/to/terms",
